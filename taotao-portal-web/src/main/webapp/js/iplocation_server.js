...
//    var traceurl = ("https:" == document.location.protocol ? "https://mercuryssl" : "http://mercury") + ".jd.controller/log.gif"
//                    + "?t=other.000000"
//                    + "&m=UA-J2011-1"
...
//        });
    ...
    itemOver: function() {
        ...
        var urlRelated = 'http://simigoods.jd.controller/SoldOutRecJsonData.aspx?ip='+ getCookie("ipLocation") +'&wids='+ pageConfig.product.skuid;
        var urlBroswerBuy = 'http://simigoods.jd.controller/ThreeCCombineBuying/ThreeCBroswerBroswerJsonData.aspx?ip=' + getCookie('ipLocation') + '&wids=' + pageConfig.product.skuid;
        var _this = this;
    ...
    noItemNoData: function( isItemOver ) {
        ...
        // 没有推荐数据时调用新数据接口
        var url = 'http://simigoods.jd.controller/SoldOutRecJsonData.aspx?ip='+ getCookie("ipLocation") +'&wids='+ pageConfig.product.skuid,
            _this = this;
...
var stockServiceDomain = "http://st.3.cn",rmsurl="http://rms.shop.jd.controller",dcashurl="http://yfei.shop.jd.controller/json/pop/fare.action";
...
function setNewCookie(name,value,expires,path,domain,secure){if(!path){path="/"}if(!domain){domain="jd.controller"}if(!secure){secure=false}var today=new Date();today.setTime(today.getTime());if(expires){expires=expires*1000*60*60*24}var expires_date=new Date(today.getTime()+(expires));document.cookie=name+'='+escape(value)+((expires)?';expires='+expires_date.toGMTString():'')+((path)?';path='+path:'')+((domain)?';domain='+domain:'')+((secure)?';secure':'')};function deleteCookie(name,path,domain){if(getCookie(name))document.cookie=name+'='+((path)?';path='+path:'')+((domain)?';domain='+domain:'')+';expires=Thu, 01-Jan-1970 00:00:01 GMT'};
...
$.getShopUrl=function(r){if(pageConfig.product.isFlashPurchase)return "#none";if(r.url)return r.url;return "http://mall.jd.controller/index-"+r.id+".html";};
...
function flashPurchaseChange(){
	...
	if($('#summary-countdown').length==0){
		$.ajax({
			url: 'http://rms.shop.360buy.controller/json/upDown/queryWareTaskUpDownTimeBySkuId.action?skuId=' + pageConfig.product.skuid,
			dataType: 'jsonp',
			success: function(r) {
...
function flashPurchaseChange1(){
	...
	if($('#summary-countdown').length==0){
		$.ajax({
			url: 'http://red.jd.controller/item/remainTime.html?skuid=' + pageConfig.product.skuid,
			dataType: 'jsonp',
			success: function(r) {
				if(r){
...
function getDeliveCash(r){
    ...
    if(r){
        if(r.dtype == 0&&new Number(r.dcash)>0){
            $("#store-prompt span").html("，运费："+r.dcash+"元 <a href='http://help.jd.controller/help/question-892.html#help2215' class='free_delivery_policy' title='了解配送费收取标准'></a>");
        }
        else if(r.dtype == 1&&new Number(r.dcash)>0&&new Number(r.ordermin)>0){
            $("#store-prompt span").html("，店铺单笔订单不满"+r.ordermin+"元，收运费："+r.dcash+"元 <a href='http://help.jd.controller/help/question-892.html#help2215' class='free_delivery_policy'></a>");
        }
        if(typeof $.fn.Jtips !== 'undefined'){
             $('.free_delivery_policy').Jtips({
                ...
                oLeft: -8,
                position:'bottom',
                content:'<a href="http://help.jd.controller/help/question-892.html#help2215" target="_blank">了解配送费收取标准</a>',
                event: 'mouseover',
                close: true
            });
        }
    ...
    if (requestDeliveCash){
        try{
			if(dcashurl){
				...
				$.getJSONP("http://fare.shop.jd.controller/json/pop/fare.action?venderId="+pageConfig.product.venderId+"&skuId="+pageConfig.product.skuid+"&provinceId="+currentAreaInfo.currentProvinceId+"&cityId="+currentAreaInfo.currentCityId+"&buyNum="+$("#buy-num").val()+"&callback=getDeliveCash");
			}
        }catch(e){}
    }
}
...
function getBuyUrl(skuId){
    ...
    if(!count)count=1;
	if(pageConfig.product.isHYKHSP)return "http://eve.jd.controller/redirect.action?wid="+skuId+"&btype=30";
	if(pageConfig.product.isLOC){
		return "http://cart.jd.controller/cart/dynamic/gateForSubFlow.action?wids="+skuId+"&nums="+count+"&subType=22";
	}
    if(eleSkuIdKey) return "http://gate.jd.controller/InitCart.aspx?pid="+skuId+"&pcount="+count+"&ptype=1";
    if($.pbuyurl)return $.pbuyurl;
    if($.append_button.attr("href")!="#none")return $.append_button.attr("href");
    if (pageConfig.product.cat[2] == 4833) return "http://chongzhi.jd.controller/order/order_place.action?skuId=" + skuId + "";
    if (pageConfig.product.cat[2] == 4835 || pageConfig.product.cat[2] == 4836) return "http://card.jd.controller/order/order_place.action?skuId=" + skuId + "";
    return "http://gate.jd.controller/InitCart.aspx?pid="+skuId+"&pcount="+count+"&ptype=1";
}
function chooseType() {
...
function setGiftTips (isGift, areaText) {
	if(checkApecialAttr("isSupportCard")){
		...
		var giftTipsHtml = (areaText || '北京地区') + '支持礼品包装' + '<a href="http://cart.gift.jd.controller/cart/addGiftToCart.action?pid='+pageConfig.product.skuid+'&pcount=1&ptype=1" target="_blank">&nbsp;&nbsp;详情 <s class="s-arrow">&gt;&gt;</s></a>';
		if (isGift) {
...
function setOldForNew(proId,cityId){
	...
	if (pageConfig.product.cat[2]==655){
		if ($("#oldfornew-tips").length==0){
			summary_tips.find(".dd").append("<div id=\"oldfornew-tips\" class=\"hl_red\">以旧换新，最高抵扣3000元<a target=\"_blank\" href=\"http://huishou.jd.controller\" clstag=\"shangpin|keycount|product|huishou\">&nbsp;&nbsp;详情<s class=\"s-arrow\">>></s></a></div>");
		}else{
			$("#oldfornew-tips").show();
		}
		summary_tips.show();
...
function getChildAreaHtml(fid,level){
    ...
    if (idName){
        ...
        if(level == 2 && provinceCityJson[fid+""]){
            ...
            $.getJSONP("http://d.360buy.controller/area/get?fid="+fid+"&callback=getAreaListcallback");
        }
    }
}
...
function aboutSelfDeliveCash(type,flag){
    if((pageConfig.product.skuid+"").length<10||flag){
        if(dCashDescInfo.loadPriceCnt==dCashDescInfo.loadStockCnt&&dCashDescInfo.bigger39&&!dCashDescInfo.dCash){
            if(type == 1){ //stock
                return '<a href="http://help.jd.controller/help/question-892.html#help2215" target="_blank" class="free_delivery" title="了解配送费收取标准"></a>';
            }
            else if (type == 2){ //price
                if($("#promise-ico").length>0){
                    if($("#promise-ico .free_delivery").length==0){
                        $('<a href="http://help.jd.controller/help/question-892.html#help2215" target="_blank" class="free_delivery" title="了解配送费收取标准"></a>').appendTo("#promise-ico");
                    }
                }
                else{
                    $('<span id="promise-ico">支持：<a href="http://help.jd.controller/help/question-892.html#help2215" target="_blank" class="free_delivery" title="了解配送费收取标准"></a></span>').appendTo("#summary-service .dd");
                }
            }
        }
    }
...
function easybuysubmit(r){
	if(r&&r.success&&r.jumpUrl){
		...
		$.jdThickBox({
			...
			height: 200,
			title: '提示',
			source: '<div style="padding:10px"><img class="fl" style="margin-right:10px;" src="http://misc.360buyimg.controller/jd2008/skin/df/i/remind.png" /><span style="font:bold 16px arial;color:#005ea8">'+r.message+'</span></div>'
		});
...
var checkLogin = function(cb) {
	...
	$.getJSON('http://passport.jd.controller/loginservice.aspx?method=Login&callback=?', function(r) {
		if ( r.Identity ) {
			cb(r.Identity);
		}
	});
						...
						$.extend(jdModelCallCenter.settings, {
							...
							fn: function () {
								$.login({
									...
									complete: function (result) {
										if (result != null && result.IsAuthenticated != null && result.IsAuthenticated) {								
											$.getJSONP("http://easybuy.jd.controller/skuDetail/submitEasybuyOrder.action?callback=easybuysubmit&skuId="+pageConfig.product.skuid+"&num="+$("#buy-num").val());
										}
										else{
											$("#btn-easybuy-submit").show();
											$("#btn-easybuy-submit-ing").hide();
    ...
    if(r&&r.resultCode==1&&r.skuId&&r.skuId!=pageConfig.product.skuid+''){
		...
		if((r.skuId+"").length==10){
			if($("#local-seller").length==0){
				$('<div class="mt" id="local-seller"><span class="fl"><b>本地在售卖家</b></span></div><div class="mc" id="local-seller1"><ul><li id="J_'+r.skuId+'"><div class="fl"><a target="_blank" href="http://item.jd.controller/'
			...
			$('<div class="mt" id="jd-seller"><span class="fl"><b>其它在售卖家</b></span></div><div class="mc" id="jd-seller1"><ul><li id="J_'+r.skuId+'"><div class="fl"><a target="_blank" href="http://item.jd.controller/'
        ...
        try{
            ...
            if (curLevel == requestLevel){
                ...
                if(pageConfig.product.TJ=='1'&&provinceId>0&&cityId>0&&areaId>0){
                    $.getJSONP("http://d.360buy.controller/goodsrelation/get?callback=getTJAreaSku&skuId="+pageConfig.product.skuid+"&provinceId="+provinceId+"&cityId="+cityId+"&countryId="+areaId);
                }
...
var spuServiceUrl = "http://spu.jd.controller/json.html?cond=";
var spuPageUrl = "http://spu.jd.controller/"+pageConfig.product.skuid+".html";
...
function getImgFullPath(skuid,src,type){
    if(src&&src.toUpperCase().indexOf("HTTP:")==-1){
        src = "http://img1"+(new Number(skuid))%5+".360buyimg.controller/n"+type+"/"+src;
    }
    return src;
}
(function(){
    ...
    if(window.itemShopJsona&&itemShopJsona.searchLink){
        var shopSearchHtml='<div id="sp-searche" class="m m2" clstag="shangpin|keycount|product|pop-03">'
    ...
    if(window.itemShopJsonb&&itemShopJsonb.hotSale&&itemShopJsonb.hotSale.length==5){
        ...
        for (var i=0,j=itemShopJsonb.hotSale.length;i<j;i++){
            hotSaleContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://item.jd.controller/'+itemShopJsonb.hotSale[i].skuId+'.html" title="'+itemShopJsonb.hotSale[i].goodsName+'" target="_blank"><img alt="'+itemShopJsonb.hotSale[i].goodsName+'" data-lazyload="'+getImgFullPath(itemShopJsonb.hotSale[i].skuId,itemShopJsonb.hotSale[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://item.jd.controller/'+itemShopJsonb.hotSale[i].skuId+'.html" target="_blank" title="'+itemShopJsonb.hotSale[i].goodsName+'">'+itemShopJsonb.hotSale[i].goodsName+'</a></div>'
    ...
    if(window.itemShopJsonb&&itemShopJsonb.hotFocus&&itemShopJsonb.hotFocus.length==5){
        ...
        for (var i=0,j=itemShopJsonb.hotFocus.length;i<j;i++){
            hotFocusContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://item.jd.controller/'+itemShopJsonb.hotFocus[i].skuId+'.html" title="'+itemShopJsonb.hotFocus[i].goodsName+'" target="_blank"><img alt="'+itemShopJsonb.hotFocus[i].goodsName+'" data-lazyload="'+getImgFullPath(itemShopJsonb.hotFocus[i].skuId,itemShopJsonb.hotFocus[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://item.jd.controller/'+itemShopJsonb.hotFocus[i].skuId+'.html" target="_blank" title="'+itemShopJsonb.hotFocus[i].goodsName+'">'+itemShopJsonb.hotFocus[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
    ...
    if(window.itemShopJsona&&itemShopJsona.shopRec&&itemShopJsona.shopRec.length>0){
        ...
        for (var i=0,j=itemShopJsona.shopRec.length;i<j;i++){
            shopRecContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://item.jd.controller/'+itemShopJsona.shopRec[i].skuId+'.html" title="'+itemShopJsona.shopRec[i].goodsName+'" target="_blank"><img alt="'+itemShopJsona.shopRec[i].goodsName+'" data-lazyload="'+getImgFullPath(itemShopJsona.shopRec[i].skuId,itemShopJsona.shopRec[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://item.jd.controller/'+itemShopJsona.shopRec[i].skuId+'.html" target="_blank" title="'+itemShopJsona.shopRec[i].goodsName+'">'+itemShopJsona.shopRec[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
